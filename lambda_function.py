...
class CodeGenerationRequest(BaseModel):
    ...
    model: Optional[str] = Field("gemini/gemini-2.5-flash-lite", description="Specific model to use")
...
def developer_main(language: str, query: str, llm, auth_token: Optional[str]):
    ...
    try:
        ...
        tester_agent = Agent(
            role="Code Tester",
            goal=f"Write comprehensive test cases for the provided code in {language} for {{requirements}}. Test functionality and edge cases without modifying the main code.",
            backstory="You are a diligent tester who creates thorough test suites to validate code functionality.",
            memory=True,
            verbose=True,
            llm=llm
        )
        ...
        # testing_task = Task(
        #     description="Create comprehensive test cases for the main code. Keep tests separate from the main implementation.",
        #     expected_output="Complete test suite with test results validation.",
        #     agent=tester_agent
        # )
...
async def generate_code(
    ...
    try:
        logger.info(f"Received code generation request for {payload.language}: {payload.query[:50]}...")
        ...
        if not payload.language.strip() or not payload.query.strip():
            raise HTTPException(status_code=400, detail="Language and query cannot be empty")
        ...
        generated_code = await asyncio.to_thread(developer_main, payload.language, payload.query, llm, authorization)

        logger.info("Code generation request successful.")
        # logger.info("here's the final code that is being generated", generated_code)
        return CodeGenerationResponse(success=True, code=generated_code, message="Code generated successfully")
        ...
        return CodeGenerationResponse(success=False, error=str(e), message="Failed to generate code")
